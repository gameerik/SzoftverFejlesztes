...
/**
 * Class represent a tool for vertical rendering of a 3x1{@link Model.GameComponents.Cell}(Domino).
 */
public class fillVerticalDomino implements fillDomino {
    ...
    public void draw(GraphicsContext gc, double centerX, double centerY, double width, double height) {
        ...
        gc.fillRect(centerX - width / 2, centerY - 1.5 * height, width, 3 * height);
        for (double j = -1.5; j < 1.5; j++) {
            gc.strokeRect(centerX - width / 2, centerY + j * height, width, height);
        }
    }
